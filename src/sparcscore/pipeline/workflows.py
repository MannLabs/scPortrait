...
class BaseSegmentation(Segmentation):
    """
    Base class for segmentation workflows.

    This class provides the basic functionality for segmentation workflows. 

    Args:
        config (dict): Dictionary containing the config file for the segmentation workflow.
        directory (str): Path to the directory where the segmentation results should be saved.
        identifier (str): Identifier of the shard which should be processed. If this class is used as part of a project processing workflow, this argument will be provided.
        debug (bool): If set to True, additional debug information will be generated.
        log (function): Function used for logging. If this class is used as part of a project processing workflow, this argument will be provided.
    """
    ...
    def _nucleus_thresholding(self):
        """
        Generate nucleus segmentation.

        The segmentation is generated by percentile normalization of the DAPI channel and subsequent thresholding.
        The threshold can be either defined manually or calculated using global otsu. The segmentation is then dilated to remove small artifacts.
        """
    ...
    def _nucleus_mask_generation(self):
        """
        Generate a mask for the nucleus segmentation.

        The mask is generated by clipping the nucleus segmentation between 0 and 1.
        """
    ...
    def _filter_nuclei_classes(self):
        ...
        The size is calculated by the number of pixels in the nucleus segmentation and limited by the parameters "min_size" and "max_size" in the config file.
        The contact is calculated by the distance between the nuclei and limited by the parameter "contact_filter" in the config file.
        """
        ...
        # ids of all nuclei which are unconnected and can be used for further analysis
    ...
    def _cellmembrane_mask_calculation(self):
        """
        Generate a mask for the cell membrane.

        The mask is generated by percentile normalization of the WGA channel and subsequent thresholding. 
        The threshold can be either defined manually or calculated using global otsu. 
        The mask is then dilated and eroded to remove small artifacts.
        """
        ...
        if "threshold" in self.config["wga_segmentation"]:
            ...
            self.log(
                "No threshold for cytosol segmentation defined, global otsu will be used."
            )
    ...
    def _cellmembrane_potential_mask(self):
        """
        Generate a potential map for the cell membrane based on the WGA channel.

        The potential map is generated by percentile normalization of the WGA channel and subsequent thresholding.
        The threshold can be either defined manually or calculated using global otsu.
        The potential map is then enhanced by subtracting the nucleus (DAPI) and golgi channels.
        """
    ...
    def _cellmembrane_fastmarching(self, center_nuclei):
        """
        Generate a cytosol segmentation based on the WGA channel.

        The segmentation is generated by performing fast marching on the WGA potential map.
        The nuclei are used as starting points for the fast marching.
        """
    ...
    def _cellmembrane_watershed(self, center_nuclei):
        ...
        The segmentation is generated by performing watershed on the WGA potential map.
        The nuclei are used as starting points for the watershed.
        """
    ...
    def _filter_cells_cytosol_size(self, all_classes, filtered_classes):
        """
        Filter cells based on the cytosol size.

        The cytosol size is calculated by the number of pixels in the cytosol segmentation.
        The segmentation is generated by the watershed algorithm.
        The cytosol size is limited by the parameters "min_size" and "max_size" in the config file.
        """
...
class DAPISegmentationCellpose(BaseSegmentation):
    ...
    def cellpose_segmentation(self, input_image):
        """
        Generate a nucleus segmentation based on the DAPI channel.

        The segmentation is generated by running Cellpose on the DAPI channel.
        """
        gc.collect()
...
class CytosolSegmentationCellpose(BaseSegmentation):
    ...
    def cellpose_segmentation(self, input_image):
        """
        Generate nucleus and cytosol segmentations.

        This function generates nucleus and cytosol segmentations by running Cellpose on the respective channels.
        Proportions of different cytosol id's around nuclei are calculated.
        Cells that do not pass the filtering criteria set by the user in the ['filtering_threshold'] parameter are discarded.
        Nucleus and cytosol ids are matched based on the nucleus mask. 
        """
