...
class HDF5CellExtraction(ProcessingStep):
    ...
    def _initialize_tempmmap_array(self, index_len = 2):
        ...
        _tmp_single_cell_data = tempmmap.array(shape = self.single_cell_data_shape, dtype = np.float16, tmp_dir_abs_path = self._tmp_dir_path)

        if index_len == 2:
            #assign dtype int to only save the index and the cell_id
            _tmp_single_cell_index  = tempmmap.array(shape = self.single_cell_index_shape, dtype = np.int64, tmp_dir_abs_path = self._tmp_dir_path)
        else:
