...
class HDF5SingleCellDataset(Dataset):
    ...
    This class provides a convenient interface for SPARCSpy formatted hdf5 files containing 
    single cell datasets. It supports loading data from multiple hdf5 files within specified 
    directories, applying transformations on the data, and returning the required information, 
    such as label or id, along with the single cell data.

    Attributes:
        root_dir (str): Root directory where the hdf5 files are located.
        dir_labels (list[int]): List of labels corresponding to the directories in dir_list.
        dir_list (list[str]): List of path(s) where the hdf5 files are stored. Supports 
            specifying a path to a specific hdf5 file or directory containing hdf5 files.
        transform (callable, optional): A user-defined function to apply transformations to the data. Default is None.
        max_level (int, optional): Maximum levels of directory to search for hdf5 files. Default is 5.
        return_id (bool, optional): Whether to return the index of the cell with the data. Default is False.
        return_fake_id (bool, optional): Whether to return a fake index (0) with the data. Default is False.
        select_channel (int, optional): Specify a specific channel to select from the data. Default is None, which returns all channels.

    Methods:
        add_hdf_to_index(current_label, path):
            Adds single cell data from the hdf5 file located at ‘path’ with the specified 
            ‘current_label’ to the index.
        scan_directory(path, current_label, levels_left):
            Scans directories for hdf5 files and adds their data to the index with the 
            specified ‘current_label’.
        stats():
