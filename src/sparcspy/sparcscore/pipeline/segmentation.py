...
from sparcscore.processing.segmentation import sc_any, shift_labels
...
class Segmentation(ProcessingStep):
    ...
    def save_segmentation(self, channels, labels, classes):
        ...
        if self.DEFAULT_CHANNELS_NAME in hf.keys():
            ...
            self.log("channels dataset already existed in hdf5, dataset was deleted and will be overwritten.")
    ...
    def save_segmentation_zarr(self, labels=None):
        ...
        if hasattr(self, "save_zarr"):
            if self.save_zarr:
                ...
                for seg, name in zip(segmentations, segmentation_names, strict=False):
    ...
    def load_maps_from_disk(self):
        ...
        for map_index, map_name in enumerate(self.maps.keys()):
            try:
                map_path = os.path.join(self.directory, f"{map_index}_{map_name}_map.npy")

                if os.path.isfile(map_path):
                    map = np.load(map_path)
                    self.log(f"Loaded map {map_index} {map_name} from path {map_path}")
                    self.maps[map_name] = map
                else:
                    self.log(
                        f"No existing map {map_index} {map_name} found at path {map_path}, new one will be created"
                    )
                    self.maps[map_name] = None

            except Exception:
                self.log(f"Error loading map {map_index} {map_name} from path {map_path}")
                self.maps[map_name] = None
    ...
    def save_map(self, map_name):
        ...
        if self.maps[map_name] is None:
            ...
            if self.intermediate_output:
                ...
                self.log(f"Saved map {map_index} {map_name} under path {map_path}")
...
class ShardedSegmentation(Segmentation):
    ...
    def initialize_shard_list_incomplete(self, sharding_plan, incomplete_indexes):
        ...
        for i, window in zip(incomplete_indexes, sharding_plan, strict=False):
            local_shard_directory = os.path.join(self.shard_directory, str(i))
            current_shard = self.method(
                self.config,
                local_shard_directory,
    ...
    def resolve_sharding(self, sharding_plan):
        ...
        with h5py.File(output, "a") as hf:
            ...
            for i, window in enumerate(sharding_plan):
                ...
                cr = csv.reader(open(local_classes))
                filtered_classes = [int(el[0]) for el in list(cr)]
                filtered_classes_combined += [
                    class_id + class_id_shift for class_id in filtered_classes if class_id != 0
                ]
            ...
            if self.debug:
                ...
                for i in range(len(hdf_labels)):
                    image = label2rgb(
                        hdf_labels[i],
                        hdf_channels[0].astype(np.float64) / np.max(hdf_channels[0].astype(np.float64)),
                        alpha=0.5,
                        bg_label=0,
                    )
                    plot_image(image)
    ...
    def complete_segmentation(self, input_image):
        ...
        if self.config["shard_size"] >= np.prod(self.image_size):
            ...
            with open(f"{self.directory}/sharding_plan.csv") as f:
    ...
    def adjust_segmentation_indexes(self):
        ...
        with h5py.File(path, "a") as hf:
            ...
            for i in tqdm(
                ...
                filtered_classes = set(np.unique(shifted_map[0])) - {0}  # remove background class
