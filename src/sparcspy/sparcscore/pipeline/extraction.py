...
from sparcscore.processing.preprocessing import MinMax, percentile_normalization
...
class HDF5CellExtraction(ProcessingStep):
    ...
    def _extract_classes(self, input_segmentation_path, px_center, arg):
        ...
        if save_index % 10000 == 0:
            self.log(f"Extracting dataset {save_index}")
    ...
    def process(self, input_segmentation_path, filtered_classes_path=None):
        ...
        if isinstance(class_list[0], str):
            ...
            nuclei_ids = {str(x) for x in class_list}
    ...
    def process_partial(self, input_segmentation_path, filtered_classes_path=None, n_cells=100):
...
class TimecourseHDF5CellExtraction(HDF5CellExtraction):
    ...
    def _transfer_tempmmap_to_hdf5(self):
        ...
        with h5py.File(self.output_path, "w") as hf:
            self.log("Transferring extended labelling to ['single_cell_index_labelled'] container.")
            ...
            self.log("Transferring extracted single cells to ['single_cell_data'] container.")
            _, c, x, y = _tmp_single_cell_data.shape
            single_cell_data = hf.create_dataset(
                "single_cell_data",
                shape=(len(keep_index), c, x, y),
        ...
        with h5py.File(self.output_path, "a") as hf:
            self.log("Transferring simple cell_id index to ['single_cell_index'] container.")
            ...
            index = np.array(list(zip(range(len(cell_ids)), cell_ids, strict=False)))
