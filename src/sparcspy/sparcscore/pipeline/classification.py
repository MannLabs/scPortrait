...
class MLClusterClassifier(ProcessingStep):
        ...
        if hasattr(self, "filtered_dataset"):
            if self.filtered_dataset is not None:
                self.run_path = os.path.join(
                    self.directory,
                    str(self.current_run) + "_" + self.config["screen_label"] + "_" + self.filtered_dataset,
                )
            else:
                self.run_path = os.path.join(
                    self.directory,
    ...
    def __call__(
        ...
        if network_dir in [
            ...
            checkpoints = [
                name for name in os.listdir(checkpoint_path) if os.path.isfile(os.path.join(checkpoint_path, name))
            ]
            checkpoints = [
...
class CellFeaturizer(ProcessingStep):
    ...
    def calculate_statistics(self, img, channel=-1):
        ...
        summed_intensity_nucleus_area = (
            masked_tensor(img_selected, nucleus_mask).view(N, -1).sum(1).reshape((N, 1)).to_tensor(0)
        )
