...
from sparcscore.processing.filtering import MatchNucleusCytosolIds, SizeFilter
from sparcscore.processing.preprocessing import downsample_img, percentile_normalization
...
class BaseSegmentation(Segmentation):
    ...
    def _nucleus_thresholding(self):
        ...
        if "threshold" in self.config["nucleus_segmentation"] and "median_block" in self.config["nucleus_segmentation"]:
            ...
            self.log("No threshold or median_block for nucleus segmentation defined, global otsu will be used.")
    ...
    def _filter_nuclei_classes(self):
        ...
        self.log(f"Filtered out due to contact limit: {len(all_classes) - len(classes_nuclei_unconnected)} ")
        ...
        self.log(f"Filtered out due to size limit: {len(all_classes) - len(classes_nuclei_filtered)} ")

        filtered_classes = set(classes_nuclei_unconnected).intersection(set(classes_nuclei_filtered))
        self.log(f"Filtered out: {len(all_classes) - len(filtered_classes)} ")
    ...
    def _filter_cells_cytosol_size(self, all_classes, filtered_classes):
        ...
        self.log(f"Cells filtered out due to cytosol size limit: {len(all_classes_wga) - len(classes_wga_filtered)} ")

        filtered_classes_wga = set(classes_wga_filtered)
        filtered_classes = set(filtered_classes).intersection(filtered_classes_wga)
        self.log(f"Filtered out: {len(all_classes) - len(filtered_classes)} ")
        self.log(f"Remaining: {len(filtered_classes)} ")
    ...
    def _finalize_segmentation_results(self):
        ...
        if "wga_background_image" in self.config["wga_segmentation"]:
            if self.config["wga_segmentation"]["wga_background_image"]:
                ...
                feature_maps = list(self.maps["normalized"][2:-1])
            else:
                feature_maps = list(self.maps["normalized"][2:])
        else:
            feature_maps = list(self.maps["normalized"][2:])

        channels = np.stack(required_maps + feature_maps).astype(np.float64)

        segmentation = np.stack([self.maps["nucleus_segmentation"], self.maps["watershed"]]).astype(np.uint64)
...
class DAPISegmentation(BaseSegmentation):
    ...
    def _finalize_segmentation_results(self):
        ...
        # Feature maps are all further channel which contain phenotypes needed for the classification
        feature_maps = list(self.maps["normalized"][1:])

        channels = np.stack(required_maps + feature_maps).astype(np.float64)

        segmentation = np.stack([self.maps["nucleus_segmentation"], self.maps["nucleus_segmentation"]]).astype(
            np.uint64
        )
        return (channels, segmentation)
...
class CytosolSegmentationDownsamplingCellpose(CytosolSegmentationCellpose):
    ...
    def _finalize_segmentation_results(self, size_padding):
        ...
        if len(np.unique(nuc_seg)) != n_nuclei:
            ...
            self.log(
                f"Recalculation of nucleus mask successful with smoothing kernel size of {smoothing_kernel_size_nuc}."
            )
        ...
        if len(np.unique(cyto_seg)) != n_cytosols:
            ...
            self.log(
                f"Recalculation of cytosol mask successful with smoothing kernel size of {smoothing_kernel_size_cytosol}."
            )
...
class CytosolOnlySegmentationCellpose(BaseSegmentation):
    ...
    def cellpose_segmentation(self, input_image):
        ...
        self.maps["cytosol_segmentation"] = masks.reshape(masks.shape[1:])  # add reshape to match shape to HDF5 shape
