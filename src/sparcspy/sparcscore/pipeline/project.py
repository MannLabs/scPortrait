...
from sparcscore.pipeline.utils import _generate_mask_polygon, _read_napari_csv
from sparcscore.processing.preprocessing import EDF, maximum_intensity_projection, percentile_normalization
...
class Project(Logable):
    ...
    channel_colors = ["#0000FF", "#00FF00", "#FF0000", "#FFE464", "#9b19f5", "#ffa300", "#dc0ab4", "#b3d4ff", "#00bfa0"]
        ...
        **kwargs,
        ...
        if segmentation_filtering_f is not None:
            if segmentation_filtering_f.__name__ not in self.config:
                raise ValueError(f"Config for {segmentation_filtering_f.__name__} is missing from the config file")

            filter_seg_directory = os.path.join(self.project_location, self.DEFAULT_SEGMENTATION_FILTERING_DIR_NAME)
    ...
    def save_input_image(self, input_image):
        ...
        write_image(input_image, group=group, axes="cyx", storage_options={"chunks": (1, 1024, 1024)})
    ...
    def load_input_from_file(self, file_paths, crop=None):
        ...
        if not len(file_paths) == self.config["input_channels"]:
            raise ValueError(
                "Expected {} image paths, only received {}".format(self.config["input_channels"], len(file_paths))
            )
    ...
    def load_input_from_array(self, array, remap=None):
        ...
        if os.path.isdir(path):
            if self.overwrite:
                ...
                raise ValueError(
                    "Overwrite is set to False but an input image already written to file. Either set overwrite = False or remove the existing input image "
                )

        if not array.shape[0] == self.config["input_channels"]:
            raise ValueError(
                "Expected {} image paths, only received {}".format(self.config["input_channels"], array.shape[0])
            )
    ...
    def load_input_from_czi(self, czi_path, intensity_rescale=True, scene=None, z_stack_projection=None, remap=None):
        ...
        if "Z" in dimensions:
            ...
            if isinstance(z_stack_projection, int):
                ...
                _mosaic = np.array(
                    [
                        czi.read_mosaic(region=(box.x, box.y, box.w, box.h), C=c, Z=z_stack_projection).squeeze()
            ...
            _mosaic = np.array(
                [czi.read_mosaic(region=(box.x, box.y, box.w, box.h), C=c).squeeze() for c in range(channels)]
            )

        # perform intensity rescaling before loading images
        if intensity_rescale:
            self.log(
                "Performing percentile normalization on the input image with lower_percentile=0.005 and upper_percentile=0.995"
            )
            _mosaic = np.array(
                [
                    percentile_normalization(_mosaic[i], lower_percentile=0.005, upper_percentile=0.995)
    ...
    def classify(self, *args, **kwargs):
        ...
        if hasattr(self, "filtered_dataset"):
            input_extraction = self.extraction_f.get_output_path().replace(
        ...
        if not os.path.isdir(input_extraction):
            raise ValueError(f"input was not found at {input_extraction}")
    ...
    def write_segmentation_to_omezarr(self):
        ...
        input_dir = os.path.join(self.project_location, self.DEFAULT_SEGMENTATION_DIR_NAME, "segmentation.h5")

        output_file = os.path.join(self.project_location, self.DEFAULT_SEGMENTATION_DIR_NAME, "segmentation.ome.zarr")
...
class TimecourseProject(Project):
    ...
    def load_input_from_files(self, input_dir, channels, timepoints, plate_layout, img_size=1080, overwrite=False):
        ...
        if overwrite:
            ...
            def _read_write_images(dir, indexes, h5py_path):
                ...
                well = (
                    re.search("Row.._Well[0-9][0-9]", dir).group()
                )  # need to use re.search and not match sinde the identifier is not always at the beginning of the name
            ...
            # read plate layout
            plate_layout = pd.read_csv(plate_layout, sep=r"\s+|;|,", engine="python")
            ...
            _wells = [re.search("Row.._Well[0-9][0-9]", _dir).group() for _dir in directories]
            ...
            with h5py.File(path, "w") as hf:
                ...
                # create indexes
                start_index = 0
                for _i, _ in enumerate(directories):
                ...
                for dir, index in tqdm(zip(directories, indexes, strict=False), total=len(directories)):
    ...
    def load_input_from_stitched_files(
        ...
        if overwrite:
            ...
            def _read_write_images(well, indexes, h5py_path):
                ...
                # checkt to make sure all timepoints are actually there
                _timepoints = np.unique([re.search("Timepoint[0-9][0-9][0-9]", x).group() for x in _files])
                ...
                # read images for that region
                imgs = np.empty((n_timepoints, n_channels, size1, size2), dtype="uint16")
                for ix, channel in enumerate(channels):
                    ...
                    for i, im in enumerate(images):
                        ...
                        if image.shape[0] < size1 or image.shape[1] < size2:
                            image = np.pad(
                                image,
                                ((0, np.max((size1 - image.shape[0], 0))), (0, np.max((size2 - image.shape[1], 0)))),
                                mode="constant",
                                constant_values=0,
                            )
                            self.log(
            ...
            # filter directories to only contain those listed in the plate layout
            files = [_dir for _dir in files if re.search("Row.._Well[0-9][0-9]", _dir).group() in wells]
            ...
            with h5py.File(path, "w") as hf:
                ...
                for well, index in tqdm(zip(wells, indexes, strict=False), total=len(wells)):
    ...
    def load_input_from_files_and_merge(
        ...
        if overwrite:
            ...
            _timepoints = np.sort(list({x.split("_")[0] for x in images}))
            _wells = np.sort(list({re.match(".*_Row[0-9][0-9]_Well[0-9][0-9]", x).group()[13:] for x in images}))
