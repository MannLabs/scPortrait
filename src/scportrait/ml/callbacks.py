...
class BatchAccumulatedMetricsCallback(Callback):
    ...
    def on_train_batch_end(
        ...
        self.train_actual_labels = torch.cat(
            (self.train_actual_labels, outputs["actual_labels"].detach().cpu())
        )
        self.train_probabilities = torch.cat(
            (self.train_probabilities, outputs["probabilities"].detach().cpu())
        )
    ...
    def on_validation_batch_end(
        ...
        self.val_actual_labels = torch.cat(
            (self.val_actual_labels, outputs["actual_labels"].detach().cpu())
        )
        self.val_probabilities = torch.cat(
            (self.val_probabilities, outputs["probabilities"].detach().cpu())
        )
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        precision, recall, thresholds = precision_recall_curve(
            labels, probs_1d, pos_label=1
        )
        auc_precision_recall = auc(recall, precision)

        self.log(
            "auc_precision_recall/val_accumulated", auc_precision_recall, sync_dist=True
        )
        ...
        if (
            self.iteration % (1 / trainer.val_check_interval) * self.n_epochs_big_calcs
            == 0
        ):
            # plot roc curve
            ...
            trainer.logger.experiment.log(
                {"roc_curve/val_accumulated": table, "t_epoch": trainer.current_epoch}
            )
            ...
            trainer.logger.experiment.log(
                {
                    "precision_recall_curve/val_accumulated": table,
                    "t_epoch": trainer.current_epoch,
                }
            )
    ...
    def on_test_batch_end(
        ...
        self.test_actual_labels = torch.cat(
            (self.test_actual_labels, outputs["actual_labels"].detach().cpu())
        )
        self.test_probabilities = torch.cat(
            (self.test_probabilities, outputs["probabilities"].detach().cpu())
        )
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        self.log(
            "auc_precision_recall/test_accumulated",
        ...
        table = wandb.Table(data=data, columns=["tpr", "fpr"])
        trainer.logger.experiment.log(
            {"roc_curve/test_accumulated": table, "t_epoch": trainer.current_epoch}
        )
        ...
        trainer.logger.experiment.log(
            {
                "precision_recall_curve/test_accumulated": table,
                "t_epoch": trainer.current_epoch,
            }
        )
