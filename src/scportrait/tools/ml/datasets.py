...
class _HDF5SingleCellDataset(Dataset):
    ...
    def __init__(
        ...
        assert (not self.return_id and self.return_fake_id), "either return_id or return_fake_id should be set but not both."
    ...
    def _add_hdf_to_index(self, 
        ...
        try:
            ...
            if read_label:
                ...
                label_col = input_hdf.get("single_cell_index_labelled").asstr()[:, label_col]  
                
                #dirty fix that we have some old datasets that have empty strings instead of nan
                if len(label_col[label_col == ""]) > 0:
                    Warning("Empty strings found in label column. Replacing with nan. Please check your single-cell dataset.")
    ...
    def _add_dataset(self, 
        ...
        if read_label_from_dataset:
            assert (self.label_column is not None), "trying to read labels from dataset but no column to access information has been passed"
            self._add_hdf_to_index(path = [path], 
            ...
            assert (self.bulk_labels is not None), "trying to apply bulk labels to all cells from dataset but no label provided"
            self._add_hdf_to_index(path = path, 
                                    index_list = current_index_list, 
    ...
    def _scan_directory(self, 
        ...
        if levels_left > 0:
            ...
            for subdirectory in current_level_directories:
                self._scan_directory(subdirectory, levels_left - 1, current_index_list, read_label_from_dataset)

        else:
            return
    
    def _add_all_datasets(self, 
                          read_label_from_dataset: bool = False):
        """
        ...
        for i, directory in enumerate(self.dir_list):
            ...
            if filetype in self.HDF_FILETYPES:
                self._add_dataset(path = directory, 
                                  current_index_list = current_index_list, 
                                  id = i,
                                  read_label_from_dataset = read_label_from_dataset)
...
class HDF5SingleCellDataset(_HDF5SingleCellDataset):
    ...
    def __init__(
        ...
        super().__init__(dir_list = dir_list,
