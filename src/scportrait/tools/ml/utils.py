...
def combine_datasets_balanced(
    ...
    if np.sum(pd.Series(class_labels).value_counts() > 1) == 0:
        for dataset, label, fraction in zip(
            list_of_datasets, class_labels, dataset_fraction
        ):
            ...
            if seed is not None:
                ...
                train, test, val, _ = torch.utils.data.random_split(
                    dataset,
                    [train_size, test_size, val_size, residual_size],
                    generator=gen,
                )
            else:
                train, test, val, _ = torch.utils.data.random_split(
...
def split_dataset_fractions(
    ...
    for dataset in list_of_datasets:
        if fractions is not None:
            if sum(fractions) != 1:
                raise ValueError(
                    "Provided fractions of the dataset should sum up to 1."
                )
            ...
            if seed is not None:
                ...
                train, test, val = torch.utils.data.random_split(
                    dataset, fractions, generator=gen
                )
            else:
