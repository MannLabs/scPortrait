...
class MultilabelSupervisedModel(pl.LightningModule):
    ...
    def __init__(self, model_type="VGG2", **kwargs):
        ...
        self.accuracy = torchmetrics.Accuracy(
            task=task_type, num_classes=self.hparams["num_classes"]
        )
        self.aucroc = torchmetrics.AUROC(
            task=task_type, thresholds=None, num_classes=self.hparams["num_classes"]
        )
...
class RegressionModel(pl.LightningModule):
    ...
    def training_step(self, batch):
        ...
        if self.hparams["loss"] == "huber":
            loss = loss_func(
                output, target, delta=self.hparams["huber_delta"], reduction="mean"
            )
        else:
        ...
        self.train_metrics = torchmetrics.MetricCollection(
            [
                torchmetrics.Precision(
                    "binary", average="none", num_classes=hparams["num_classes"]
                ),
                torchmetrics.Recall(
                    "binary", average="none", num_classes=hparams["num_classes"]
                ),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            self.logger.experiment.add_image(
                "prediction", img, self.current_epoch, dataformats="NHWC"
            )
