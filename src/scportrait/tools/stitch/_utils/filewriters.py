...
def write_ome_zarr(
    ...
    if os.path.isdir(filepath):
        if overwrite:
            ...
            raise ValueError(
                f"Outfile {filepath} already exists. Set overwrite to True to delete it."
            )
    ...
    if channel_colors is None:
        ...
        if len(channel_colors) != len(channels):
            raise ValueError(
                "Number of channel colors does not match number of channels"
            )
...
def write_xml(
    image_paths: List[str], channels: List[str], slidename: str, outdir: str = None
):
    ...
    for i, image_path in enumerate(image_paths):
        if channels[i] not in image_path:
            raise ValueError(
                f"Channel {channels[i]} is not included in image {image_path}"
            )
...
def write_spatialdata(
    ...
    if os.path.exists(image_path):
        if overwrite:
            ...
            raise ValueError(
                f"File {image_path} already exists. Set overwrite to True to delete it."
            )
    ...
    if channel_names is None:
        ...
        if len(channel_names) != image.shape[0]:
            raise ValueError(
                "Number of channel names does not match number of channels in image"
            )
