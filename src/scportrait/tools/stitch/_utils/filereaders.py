...
class FilePatternReaderRescale(FilePatternReader):
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"Error: Could not read images with the given pattern {pattern}. Please check the path {path} and pattern. Exception: {e}"
            )
            print(
                f"At the provided location the following files could be found:{os.listdir(path)} "
            )
    ...
    def correct_illumination(
        ...
        img = rescale_image(
            img, rescale_range, cutoff_threshold=cutoff_threshold, return_float=True
        )
        ...
        img_corrected = skimage.exposure.rescale_intensity(
            img + correction_lows, out_range=(0, 1)
        )
        ...
        if double_correct:
            ...
            img_corrected_double = skimage.exposure.rescale_intensity(
