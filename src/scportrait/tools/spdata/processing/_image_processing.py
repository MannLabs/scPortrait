...
def _rescale_image(img: da.Array, lower_quantiles:np.ndarray, IPRs:np.ndarray, dtype:np.dtype = np.uint16) -> da.Array:
...
def percentile_normalize_image(sdata,
                               image_name: str,
    ...
    if isinstance(image, xarray.DataTree):
        ...
        if scale_factors is None:
            # get scale factors from existing object
    ...
    write_image(sdata, image = data_rescaled, image_name = f"{image_name}_rescaled", scale_factors = [2, 4, 8], transform = local_transform)
