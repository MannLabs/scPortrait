...
def dask_array_from_path(file_path, container_name="array"):
    ...
    with h5py.File(file_path, "r") as hdf_file:
        ...
        if array.chunks is None:
            ...
            dask_array = mmap_dask_array_contigious(
                file_path, shape, dtype, offset=offset, chunks=chunks
            )
        else:
            # Chunked dataset
            chunks = calculate_chunk_sizes_chunks(
                shape, dtype, HDF5_chunk_size=array.chunks
            )
            dask_array = mmap_dask_array_chunked(
                file_path, shape, dtype, container_name, chunks
            )
