...
def _generate_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Manipulate existing single cell hdf5 datasets."
    )

    # define arguments
    parser.add_argument(
        "input_dataset", type=str, help="input dataset which should be split"
    )
    ...
    parser.add_argument(
        ...
        action="append",
        nargs=2,
        help="Output definition <name> <length>. For example -o test.h5 0.9 or or -o test.h5 1000. If the sum of all lengths is <= 1, it is interpretated as fraction. Else its used as absolute value",
    )
    parser.add_argument(
        "-r",
        "--random",
        default=False,
        action="store_true",
        help="shuffle single cells randomly",
    )
