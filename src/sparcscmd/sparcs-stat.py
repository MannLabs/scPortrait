...
def generate_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Scan directory for scPortrait projects."
    )
    ...
    parser.add_argument(
        "-r", "--recursion", type=int, default=5, help="levels of recursion"
    )
    return parser
...
def scan_directory(levels_left, path, num_threads=1):
    ...
    if levels_left > 0:
        ...
        if is_project_dir:
            ...
            segmentation_finished = os.path.isfile(
                os.path.join(path, segmentation_name)
            )

            segmentation_classes_name = "segmentation/classes.csv"
            classes_exist = os.path.isfile(
                os.path.join(path, segmentation_classes_name)
            )

            if classes_exist:
                segmentation_classes = sum(
                    1 for line in open(os.path.join(path, segmentation_classes_name))
                )
            else:
            ...
            if os.path.isdir(extraction_dir):
                current_level_files = [
                    ...
                    if os.path.isfile(os.path.join(extraction_dir, name))
                ]
                    ...
                    for i, file in enumerate(current_level_files):
                        ...
                        if filetype in HDF_FILETYPES:
                            file_path = os.path.join(
                                filtered_extraction_dir, _folder, file
                            )
                            ...
                            filtered_extractions.append(
                                (os.path.basename(_folder), length, size)
                            )
            ...
            with Pool(max_workers=num_threads) as pool:
                projects = pool.map(
                    partial(scan_directory, levels_left - 1), current_level_directories
                )
