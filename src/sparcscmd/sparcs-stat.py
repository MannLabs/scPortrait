    ...
    if levels_left > 0:
        ...
        if is_project_dir:     
            ...
            if os.path.isdir(filtered_extraction_dir):
                folders = [name for name in os.listdir(filtered_extraction_dir) if os.path.isdir(os.path.join(filtered_extraction_dir, name))]
                
                for _folder in folders:
                    folder = os.path.join(filtered_extraction_dir,_folder)
                    current_level_files = [ name for name in os.listdir(folder) if os.path.isfile(os.path.join(folder, name))]
            ...
            return [[dir_name, segmentation_finished, "{:,}".format(segmentation_classes), sizeof_fmt(project_size), extractions, filtered_extractions]]
