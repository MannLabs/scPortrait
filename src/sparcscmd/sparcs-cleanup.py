...
def generate_parser():
    ...
    parser.add_argument(
        "-r", "--recursion", type=int, default=10, help="levels of recursion"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="True",
        help="Indicate if a dry run should be performed",
    )
...
def main():
    ...
    for line in tabel:
        ...
        if dry_run == "False":
            ...
            print(
                "Deleted files with a total storage size of",
                sizeof_fmt(file_sizes + dir_sizes),
            )
...
def scan_directory_clean(levels_left, path, num_threads=10):
    if levels_left > 0:
        ...
        if is_project_dir:
            ...
            _to_delete = []
            _to_delete = _to_delete + glob.glob(
                os.path.join(path, "segmentation", "tiles", "*")
            )
            _to_delete = (
                _to_delete
                + glob.glob(os.path.join(path, "segmentation", "shards", "*"))
            )  # for backward compatability with previous runs where it was still called shards
            files = []
            dirs = []
            ...
            with Pool(max_workers=num_threads) as pool:
                projects = pool.map(
                    partial(scan_directory_clean, levels_left - 1),
                    current_level_directories,
                )
