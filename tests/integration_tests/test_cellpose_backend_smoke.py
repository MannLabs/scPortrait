...
def test_cellpose_backend_smoke_uses_real_cellpose_dependency():
    ...
    model_spec = CellposeModelSpec(model_type="pretrained", name="cpsam", gpu=False, device=torch.device("cpu"))

    try:
        model = backend.load_model(model_spec)
    except FileNotFoundError as exc:
        pytest.fail(
            f"Pretrained Cellpose integration load failed in opt-in mode ({RUN_CELLPOSE_INTEGRATION_ENV}=1): {exc}"
        )
